...
class PermissionLockdownTest extends TestCase {
	...
	public function testOnApiCheckCanExecute( $params, $userCanRead, $expectedModifiedParams ) {
		...
		$titleFactory->method( 'newFromPageIdentity' )->willReturnCallback(
			static function ( $pageIdentity ) use ( $titleFactoryReal ) {
				$title = $titleFactoryReal->newFromPageIdentity( $pageIdentity );
				return $title;
			} );
