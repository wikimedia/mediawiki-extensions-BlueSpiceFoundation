...
class BsLinkProvider {
	/**
	 * DEPRECATED!
	 * Creates an HTML anchor tag (<a>) for the UserPage of the given User object.
	 * Allows additional configuration of this tag.
	 ...
	 * @param String $sLabel If the link should have a special text instead of the users name,
	 * this parameter can contain it. By default the users real name (if set) or canonical
	 * username are used. I.e. $sLabel = 'the White Wizard'
	 * @param String $sTitle This value will be used for the anchor tags "title" attribute.
	 * If nothing is provided the $sLable text will be used. I.e. $sTitle = 'Gandalf the Grey'
	 * @param Boolean $bRelative Switch to make urls relative or absolute.
	 * @param Array $aAdditionalQueryString If - for some reason - additional querystring
	 * parameters are needed, those can be provided as an associative array.
	 * I.e. $arAdditionalQueryString = array( 'action' => 'edit', 'ring' => 'Narya') will
	 * result in something like 'href ="index.php?title=User:Gandalf$action=edit&ring=Narya"' )
	 * @param Array $aClasses Simple array of classes to be used within the "class" arrtibute
	 * of the anchor tag. I.e. $arClasses = array( 'green-underline', 'bold-text' ) will result
	 * in 'class="green-underline bold-text"'
	 * @param Array $aStyles An associative array of css style statements.
	 * I.e. $arStyles = array( 'background-color' => '#00FF00', 'margin-left' => '10px' ) will
	 * result in 'style="background-color: #00FF00; margin-left: 10px;"'
	 * @param Array $aAdditionalAttributes If special attributes are required, you can pass them
	 * in an associative array. Attention: attibutes that can be set by parameter
	 * (style, class, title, ...) are not allowed!I.e. $arAdditionalAttributes =
	 * array( 'target' => '_blank', 'name' => 'Mithrandir', 'title' => 'Not a mage' ) will
	 * result in <a href="[...]" title="Gandalf" [...] target="_blank" name="Mithrandir">[...]</a>
	 * @return String Something like
	 * '<a href="[...]index.php?title=Gandalf$action=edit" title="Gandalf" [...]>the White Wizard</a>'
	 */
	public static function makeAnchorTagForUser( User $oUser, $sLabel = '', $sTitle = '',
		...
		$sFurtherAttributes = self::makeAttributesFromArrays(
			$aClasses,
			$aStyles,
			$aAdditionalAttributes
		);
	 ...
	 * @param String $sJumpMark You can add a jumpmark to the link to go to a specific
	 * section of the page. I.e. $sJumpMark = 'vita'
	 ...
	 * @return String Something like [[Mordor#Shelob|Giant spider in Mordor]]
	 */
	public static function makeWikiLinkForTitleString( $sTitle, $sNamespaceText = '',
		$sLabel = '', $sJumpMark = '' ) {
		wfDebugLog( 'bluespice-deprecations', __METHOD__, 'private' );
	 ...
	 * @return string
	 */
	private static function makeAttributesFromArrays( $aClasses = [], $aStyles = [],
		$aAdditionalAttributes = [] ) {
		wfDebugLog( 'bluespice-deprecations', __METHOD__, 'private' );
		// TODO MRG (21.09.10 12:53): ich würde generell das Vorgehen über
		// Option-Arrays bevorzugen. Warum sollten Klassen
	 ...
	 * @return String Link
	 */
	public static function makeLink( $oTitle, $sHtml = null, $aCustomAttribs = [],
		$aQuery = [], $aOptions = [] ) {
		wfDebugLog( 'bluespice-deprecations', __METHOD__, 'private' );
