...
class HTMLForm {
	...
	// TODO SU (04.07.11 12:57): in language file auslagern
	...
	public function __construct($descriptor, $messagePrefix='') {
		...
		if (!HTMLForm::$messagesLoaded) {
			foreach ($this->mMessages as $lang => $langMessages) {
				$wgMessageCache->addMessages($langMessages, $lang);
			}
			HTMLForm::$messagesLoaded = true;
		}
	...
	function trySubmit() {
		...
		foreach ($this->mFlatFields as $fieldname => $field) {
			...
			if ($field->validate(
					...
					!== true) {
				return isset($this->mValidationErrorMessage) ? $this->mValidationErrorMessage : array('htmlform-invalid-input');
			}
		}
	...
	static function formatErrors($errors) {
		...
		foreach ($errors as $error) {
			...
			$errorstr .= Html::rawElement(
							'li', null, wfMsgExt($msg, array('parseinline'), $error)
			);
		}
	...
	function getSubmitText() {
		return $this->mSubmitText ? $this->mSubmitText : wfMsg('htmlform-submit');
	}
	...
	function displaySection($fields, $sectionName = '') {
		...
		foreach ($fields as $key => $value) {
			if (is_object($value)) {
				$v = empty($value->mParams['nodata']) ? $this->mFieldData[$key] : $value->getDefault();
...
abstract class HTMLFormField {
	...
	function getTableRow($value) {
		...
		$html .= Html::rawElement('td', array('class' => 'mw-input'), $this->getInputHTML($value) . "\n$errors");

		$fieldType = get_class($this);

		$html = Html::rawElement('tr', array('class' => "mw-htmlform-field-$fieldType"), $html) . "\n";
	...
	function getLabelHtml() {
		...
		return Html::rawElement('td', array('class' => 'mw-label'), Html::rawElement('label', $for, $this->getLabel())
		);
	}
...
class HTMLTextAreaField extends HTMLFormField {
	...
	function getCols() {
		return isset($this->mParams['cols']) ? $this->mParams['cols'] : 80;
	}

	function getRows() {
		return isset($this->mParams['rows']) ? $this->mParams['rows'] : 25;
	}
	...
	function formatOptions($options, $value) {
		...
		foreach ($options as $label => $info) {
			if (is_array($info)) {
				...
				$checkbox = Xml::check($this->mName . '[]', in_array($info, $value), $attribs + $thisAttribs);
	...
	function formatOptions($options, $value) {
		...
		foreach ($options as $label => $info) {
			if (is_array($info)) {
				...
				$html .= Xml::radio($this->mName, $info, $info == $value, $attribs + array('id' => $id));
				$html .= '&nbsp;' .
...
class HTMLHiddenField extends HTMLFormField {
	...
	public function getTableRow($value) {
		$this->mParent->addHiddenField(
				$this->mParams['name'], $this->mParams['default']
		);
		return '';
	}
...
class HTMLSubmitField extends HTMLFormField {
	...
	public function getTableRow($value) {
		$this->mParent->addButton(
				$this->mParams['name'], $this->mParams['default'], isset($this->mParams['id']) ? $this->mParams['id'] : null, $this->getTooltipAndAccessKey()
		);
	}
...
class HTMLEditTools extends HTMLFormField {
	...
	public function getTableRow($value) {
		...
		. wfMsgExt(empty($this->mParams['message']) ? 'edittools' : $this->mParams['message'], array('parse', 'content'))
		. "</div></td></tr>\n";
	}
